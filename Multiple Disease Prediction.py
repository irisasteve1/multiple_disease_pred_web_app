...
# load the scaler for diabetes model

scaler = pickle.load(open('scaler.sav', 'rb'))

# load scaler for parkinsons disease model
...
with st.sidebar:
    ...
    selected = option_menu('Multiple Disease Prediction System using ML',
                           ...
                           icons= ['activity', 'heart-pulse-fill', 'person-standing'],
...
if selected == 'Diabetes Prediction':
    ...
    if st.button('Diabetes Test Result'):
        input_data = np.array(
            [Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age]).reshape(
            1, -1)
...
if selected == 'Parkinsons Disease Prediction':
    ...
    if st.button("Parkinson's Test Result"):
        ...
        # predict
        parkinsons_prediction = parkinsons_disease_model.predict(scaled_input)
